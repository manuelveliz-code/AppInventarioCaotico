...
abstract class WebSocketServer {
  ...
  protected $userClass = 'usuarioesp'; 
  ...
  function __construct($addr, $port, $bufferLength = 2048) {
    ...
    $this->stdout("Servidor Iniciado\nEscuchando en puerto: $addr:$port\nSocket Maestro: ".$this->master);
  ...
  public  function transmitir($message,$usuario){
     foreach ($this->users as $currentUser) {
      if (($usuario->canal == $currentUser->canal)&&($usuario->id!=$currentUser->id )) {
        echo $currentUser->id .':canal:'.$currentUser->canal;
        echo '--->\n';
    ...
    while(true) {
      ...
      foreach ($read as $socket) {
        if ($socket == $this->master) {
          ...
          if ($client < 0) {
            $this->stderr("Fallido: socket_accept()");
            continue;
          } 
          else {
            $this->connect($client,'elcanal');
          ...
          if ($numBytes === false) {
            ...
            switch ($sockErrNo)
            {
              ...
              default:
                ...
                $this->disconnect($socket, true, $sockErrNo); // disconnect before clearing error, in case someone with their own implementation wants to 
                error_log('Socket error: '.$sockErrNo.' - ' . socket_strerror($sockErrNo),0);
          ...
          elseif ($numBytes == 0) {
            ...
            $this->stderr("Cliente desconectado conexión TCP perdida: " . $socket);
          } 
          else {
  ...
  protected function connect($socket,$canal) {
    $user = new $this->userClass(uniqid('u'), $socket,$canal);
  ...
  protected function doHandshake($user, $buffer) {
    ...
    $headers = array();
    $lines = explode("\n",$buffer);
    // $this->stdout(trim($buffer));
    foreach ($lines as $line) {
      if (strpos($line,":") !== false) {
        $header = explode(":",$line,2);
        $headers[strtolower(trim($header[0]))] = trim($header[1]);
        //$this->stdout(trim($header[0]).' -:- '.trim($header[1]));
      }
      elseif (stripos($line,"get ") !== false) {
        preg_match("/canal=(.*) HTTP/", $buffer, $reqResource);
        $headers['canal']= trim($reqResource[1]);

        preg_match("/GET (.*) HTTP/i", $buffer, $reqResource);
        $headers['get'] = trim($reqResource[1]);
      }
    }
    if (isset($headers['get'])||isset($headers['canal'] )) {
      $user->requestedResource = $headers['get'];
      $user->canal = $headers['canal'];
      $this->stdout("Suscrito al canal: " . $headers['canal']);
     // $this->stdout($user->requestedResource);
    } 
    else {
