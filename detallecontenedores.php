...
$idDetAgrupa= $_POST['idDetAgrupa']; // es el detalle de el grupo asignado
// si iddetagrupa viene vacio, se reconsulta con G5
if ($idDetAgrupa==0){
	$resultadoxx = $libs-> getGrupoPicking('G5', $_SESSION['grupopicking'],'',$xcunico,$xpedido,'','');  // consulta de iddetagrupa
	if ($resultadoxx) {
...
if ($resultado==true)
{
    ...
    $xdetalle=$xdetalle. '
    			...
    			<div class="input-group-append"><button class="btn btn-primary fas fa-plus" id="cmdaddcontenedor" type="button"  cunico= "'.$xcunico.'"  pedido= "'.$xpedido.'"  idDetAgrupa= "'.$idDetAgrupa.'" style="width: 68px;"></button></div>
    		</div>
	  	</div>
	</div>
	...
	$(document).ready(function() {
		$("#cmdaddcontenedor").click(function(evento) {
			...
			if (datonuevoelemento>0){
				if ($idDetAgrupa==0){
					$idDetAgrupa= grupoegreso(cunico, pedido);
					if ($idDetAgrupa>=1){
